--[[
    ╔══════════════════════════════════════════════════════════════════════╗
    ║                         kiwisense  UI Library                       ║
    ║                           Made by samet                             ║
    ╚══════════════════════════════════════════════════════════════════════╝

    ICONS  (Footagesus/Icons v2)
    ─────────────────────────────────────────────────────────────────────
    Icons are resolved automatically from a name string.
    Supported sets: lucide (default), geist, craft, solar, sfsymbols, gravityui

        icon = "zap"                   -- lucide "zap"
        icon = "geist:home"            -- geist  "home"
        icon = "sfsymbols:Star"        -- SF Symbol "Star"
        icon = "rbxassetid://12345"    -- raw asset id (passed through)
        icon = "12345"                 -- numeric id (wrapped automatically)

    SIZING
    ─────────────────────────────────────────────────────────────────────
    PC      : 659 × 511  px
    Mobile  : 720 × 320  px  (landscape-optimised)

    THEME   – defaults to "BluePink" (dark blue bg, hot-pink accent)
    ─────────────────────────────────────────────────────────────────────

    API REFERENCE
    ─────────────────────────────────────────────────────────────────────
    Library:Window({ Name, Version, Logo, Size, FadeSpeed })

    Library:Watermark(Text, Logo)
    Library:KeybindsList()
    Library:Notification({ Name, Description, Duration, Icon, IconColor })

    Window:Page({ Name, Icon, Columns })

    Page:Section({ Name, Icon, Side })          -- Side 1 = left col, 2 = right col

    Section:Toggle({ Name, Default, Flag, Callback })
    Section:Slider({ Name, Min, Max, Default, Decimals, Flag, Suffix, Callback })
    Section:Dropdown({ Name, Items, Default, MaxSize, Flag, Multi, Callback })
    Section:Button({ Name, Callback })
    Section:Label(Name, Alignment)
    Section:Textbox({ Name, Default, Placeholder, Flag, Callback })
    Section:AddImage({ Icon, Height, Color, Visible })   ← NEW

    Toggle:Keybind({ Name, Flag, Mode, Default, Callback })
    Toggle:Colorpicker({ Name, Default, Flag, Alpha, Callback })

    Label:Keybind({ Name, Flag, Mode, Default, Callback })
    Label:Colorpicker({ Name, Default, Flag, Alpha, Callback })

    NOTE: assign unique flags to each element to avoid config collisions.
]]
...
local Library do
    ...
    -- ─────────────────────────────────────────────────────────────────────────
    -- Icons V2 Integration (Footagesus/Icons)
    -- Supports: lucide, craft, geist, solar, sfsymbols, gravityui
    -- Usage:  Icon = "zap"               → lucide zap
    --         Icon = "geist:house-fill"  → geist icon
    --         Icon = "sfsymbols:Star"    → SF Symbol
    -- ─────────────────────────────────────────────────────────────────────────
    local Icons = nil
    ...
    -- Resolve an icon name/prefix string → rbxassetid:// URL
    -- Returns a plain rbxassetid string so it drops straight into Image properties.
    local function ResolveIcon(Name)
        if not Name then return nil end
        -- Already a full assetid or URL → pass through
        if type(Name) == "string" and (string.sub(Name, 1, 13) == "rbxassetid://"
            or string.sub(Name, 1, 7) == "http://"
            or string.sub(Name, 1, 8) == "https://") then
            return Name
        end
        -- Numeric string → treat as asset id
        if tonumber(Name) then
            return "rbxassetid://" .. Name
        end
        -- Named icon → resolve via Icons library
        local lib = Icons or LoadIcons()
        if lib then
            local ok, asset = pcall(lib.GetIcon, Name)
    ...
    local Themes = {
        -- ── Default – dark blue base, hot‑pink accent ─────────────────────────
        ["BluePink"] = {
            ["Background"]      = FromRGB(10,  14,  30),
            ["Inline"]          = FromRGB(18,  24,  48),
            ["Shadow"]          = FromRGB(0,   0,   0),
            ["Text"]            = FromRGB(240, 240, 255),
            ["Image"]           = FromRGB(200, 210, 255),
            ["Dark Gradient"]   = FromRGB(160, 170, 230),
            ["Inactive Text"]   = FromRGB(140, 150, 200),
            ["Element"]         = FromRGB(28,  36,  72),
            ["Accent"]          = FromRGB(255, 70,  180),
            ["Border"]          = FromRGB(36,  44,  90),
        },

        -- ── Preset (original dark) ────────────────────────────────────────────
        ["Preset"] = {
    ...
    Library.Theme = TableClone(Themes["BluePink"])
    ...
    Library.Round = function(self, Number, Float)
        -- Float is the step size (e.g. 0.1 → 1 decimal place, 1 → integer, 0 → integer)
        -- Guard: Float=0 or nil means integer rounding; 1/0 would produce inf → NaN
    ...
    do -- Element functions
        ...
        Library.Window = function(self, Data)
            ...
            local Window = {
                ...
                -- ── Sizing ────────────────────────────────────────────────────
                -- PC  : 659 × 511   (original comfortable desktop size)
                -- Mobile : 720 × 320  (landscape-optimised touch size)
                Size = not IsMobile
                    and UDim2New(0, 659, 0, 511)
                    or  UDim2New(0, 720, 0, 320),
            ...
            local Items = { } do
                ...
                Items["Pages"] = Instances:Create("Frame", {
                    ...
                    Name = "\0",
                    BorderColor3 = FromRGB(0, 0, 0),
                    AnchorPoint = Vector2New(0.5, 1),   -- anchor bottom-centre inside the frame
                    BorderSizePixel = 0,
                    Position = UDim2New(0.5, 0, 1, -8), -- 8 px gap from the very bottom
                    Size = UDim2New(0, 0, 0, 45),
                    ZIndex = 3,
                    AutomaticSize = Enum.AutomaticSize.X,
                    BackgroundColor3 = FromRGB(18, 24, 48)
                ...
                Items["Logo"] = Instances:Create("ImageLabel", {
                    ...
                    Size = UDim2New(0, 22, 0, 22),
                    AnchorPoint = Vector2New(0, 0.5),
                    Image = ResolveIcon(Window.Logo) or (tonumber(Window.Logo) and ("rbxassetid://"..Window.Logo)) or "rbxassetid://135215559087473",
                    BackgroundTransparency = 1,
                    Position = UDim2New(0, 7, 0.5, 0),
                    ZIndex = 2,
                    BorderSizePixel = 0,
        ...
        Library.Page = function(self, Data)
            ...
            local Items = { } do
                ...
                Items["Columns"] = Instances:Create("Frame", {
                    ...
                    Size = UDim2New(1, -14, 1, -101), -- y=48 offset + 45px Pages bar + 8px gap = 101
                    ZIndex = 2,
                    BorderSizePixel = 0,
                    BackgroundColor3 = FromRGB(255, 255, 255)
                })
                ...
                Items["Icon"] = Instances:Create("ImageLabel", {
                    ...
                    Size = UDim2New(0, 22, 0, 22),
                    AnchorPoint = Vector2New(0, 0.5),
                    Image = ResolveIcon(Page.Icon) or (tonumber(Page.Icon) and ("rbxassetid://"..Page.Icon)) or "rbxassetid://111178525804834",
                    BackgroundTransparency = 1,
                ...
                -- Always create scrollable columns directly – SubPage layer is removed
                ...
                Items["Inactive"].Instance.Size = UDim2New(0, 35, 0, 32)
            end
            ...
            function Page:Switch(Bool)
                ...
                if Bool then
                    ...
                    -- Wait one Heartbeat so the renderer computes TextBounds before sizing the tab
                    task.spawn(function()
                        task.wait()
                        local Bounds = Items["Text"].Instance.TextBounds.X
                        if Bounds <= 0 then Bounds = #Page.Name * 7 end
                        Items["Inactive"]:Tween(nil, {BackgroundTransparency = 0, Size = UDim2New(0, Bounds + 38, 0, 32)})
                    end)
                    ...
                    Items["Inactive"]:Tween(nil, {BackgroundTransparency = 1, Size = UDim2New(0, 35, 0, 32)})
        ...
        Library.Pages.Section = function(self, Data)
            ...
            local Items = { } do
                ...
                Items["Icon"] = Instances:Create("ImageLabel", {
                    ...
                    Size = UDim2New(0, 18, 0, 18),
                    AnchorPoint = Vector2New(1, 0.5),
                    Image = ResolveIcon(Section.Icon) or (tonumber(Section.Icon) and ("rbxassetid://"..Section.Icon)) or "rbxassetid://9080568477801",
                    BackgroundTransparency = 1,
        ...
        -- ─────────────────────────────────────────────────────────────────────
        -- Section:AddImage
        -- Renders an image (or named icon) inside a section content area.
        --
        -- Usage:
        --   MySection:AddImage({
        --       Icon   = "zap",            -- lucide / geist / sfsymbols / etc.
        --       Height = 40,               -- px height of the image row
        --       Color  = Color3.new(1,1,1),-- optional tint
        --   })
        -- ─────────────────────────────────────────────────────────────────────
        Library.Sections.AddImage = function(self, Data)
            Data = Data or {}

            local Section = self

            local IconName   = Data.Icon   or Data.icon   or nil
            local Height     = Data.Height or Data.height or 36
            local Color      = Data.Color  or Data.color  or Color3.new(1, 1, 1)
            local Visible    = Data.Visible ~= false

            -- Resolve icon → asset URL
            local ResolvedUrl = nil
            if IconName then
                ResolvedUrl = ResolveIcon(IconName)
            end
            if not ResolvedUrl and IconName and tonumber(IconName) then
                ResolvedUrl = "rbxassetid://" .. IconName
            end
            ...
            function ImgObj:SetIcon(NewIcon)
                local url = ResolveIcon(NewIcon) or ("rbxassetid://" .. tostring(NewIcon))
                ImageLabel.Instance.Image = url
            end

            function ImgObj:SetColor(NewColor)
                ImageLabel.Instance.ImageColor3 = NewColor
            end

            function ImgObj:SetHeight(NewHeight)
                Holder.Instance.Size = UDim2New(1, 0, 0, NewHeight)
            end

            function ImgObj:SetVisible(Bool)
                Holder.Instance.Visible = Bool
            end
        ...
        Library.Sections.Slider = function(self, Data)
            ...
            local Items = { } do
                ...
                -- ── Click-to-type textbox ────────────────────────────────────
                -- Appears when the user clicks/taps the value label.
                -- Type a number then press Enter (or click away) to set it.
                Items["EditBox"] = Instances:Create("TextBox", {
                    Parent = Items["Slider"].Instance,
                    Name = "\0",
                    FontFace = Library.Font,
                    TextColor3 = FromRGB(255, 255, 255),
                    ...
                    Visible = false,
                    BackgroundColor3 = FromRGB(34, 39, 45)
                })  Items["EditBox"]:AddToTheme({BackgroundColor3 = "Element", TextColor3 = "Text", PlaceholderColor3 = "Inactive Text"})

                Instances:Create("UICorner", {
                    Parent = Items["EditBox"].Instance,
                    Name = "\0",
                    CornerRadius = UDimNew(0, 4)
                })
            ...
            function Slider:Set(Value)
                if type(Value) ~= "number" or Value ~= Value then return end  -- NaN/nil guard
            ...
            Items["RealSlider"]:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    local AbsX = Items["RealSlider"].Instance.AbsoluteSize.X
                    if AbsX <= 0 then return end   -- layout not ready yet, skip

                    Slider.Sliding = true 

                    local SizeX = math.clamp((Input.Position.X - Items["RealSlider"].Instance.AbsolutePosition.X) / AbsX, 0, 1)
            ...
            -- Click the value label → open inline textbox for manual entry
            Items["Value"]:Connect("InputBegan", function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    Items["Value"].Instance.Visible = false
                    Items["EditBox"].Instance.Text = tostring(Slider.Value)
                    Items["EditBox"].Instance.Visible = true
                    Items["EditBox"].Instance:CaptureFocus()
                end
            end)

            -- Confirm manual entry on Enter or click-away
            Items["EditBox"]:Connect("FocusLost", function(EnterPressed)
                Items["EditBox"].Instance.Visible = false
                Items["Value"].Instance.Visible = true
                local Num = tonumber(Items["EditBox"].Instance.Text)
                if Num then
                    Slider:Set(Num)
                end
            end)
